...
namespace ToonPeople
{
    public class animatorshow : MonoBehaviour
    {
        ...
        GameObject newtallchair1;
        GameObject newtallchair2;
        bool phoneON;
        bool rootON;
        float elderOfset;
        float peopleofset;
        ...
        void Start()
        {
            peopleofset = 0f; flyofset = 0f;
            if (transform.childCount > 0) Destroy(transform.GetChild(0).gameObject);
            elderOfset = -0.8f;
            rootON = false;
            phoneON = false;
            animN = 0;
            setname = new string[9] { "Movement", "Idles", "Actions", "Sit down", "Tall chair", "Phone", "Dressing room" ,"Lie down","Float and fly"};
            ...
            RandomMale.transform.position += transform.right * (0.8f + peopleofset);
            RandomFemale.transform.position += transform.right * peopleofset;
            ...
            AnimText.text = animations[animN];
            SetsText.text = setname[set];
        }
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown("x")) showUIONOFF();
        ...
        void changeanimation()
        {
            ...
            if (set == 8 && animN > 8) flyofset = 1f;
            else flyofset = 0f;

            RandomMale.transform.position  = transform.position + transform.right * (0.8f + peopleofset) + Vector3.up * flyofset;
            RandomFemale.transform.position = transform.position + transform.right * peopleofset + Vector3.up * flyofset;
            RandomElder.transform.position = transform.position + transform.right * elderOfset + Vector3.up * flyofset;
        ...
        void changeset(int next)
        {
            ...
            if (set == 1) animations = new string[12] { "idle1","idle2","idle3","idle4", "idle5", "idlehappy" , "idlesad", "idleafraid", "idleangry", "idleamazed", "idleembarrassed",
                                                    "idletired" };
            if (set == 2) animations = new string[19] { "talk1","talk2","clap","wave", "salute1", "salute2" , "laugh", "cry", "telloff", "scream", "sneeze", "grabUP", "grabDOWN", "victory1",
                                                    "victory2", "defeat1", "defeat2", "yes", "no" };
            if (set == 3) animations = new string[7] { "lookback", "sitdownIN", "sitidle1", "sitidle2", "sitidle3", "sitdownmovechairIN", "sitdownmovechairOUT" };
            if (set == 4) animations = new string[3] {  "tallchairIN", "tallchairidle", "tallchairOUT" };
            if (set == 5) animations = new string[9] { "phoneidle", "phonetalk", "phonesurf", "phonehappy", "phoneangry", "phoneamazed", "phonesitidle", "phonesittalk", "phonesitsurf" };

            if (set == 6) animations = new string[21] { "DR_idle1", "DR_idle2", "DR_randomize1", "DR_randomize2", "DR_hair1", "DR_hair2",
            ...
            if (set == 7) animations = new string[7] { "liedownIN", "sunbath", "idlesleep1", "idlesleep2", "idlesleep3", "sitdownliedownRIN", "sitdownliedownLIN" };
            if (set == 8) animations = new string[16] { "float", "floatforward", "floatbackwards", "floatR", "floatL", "floatUP", "floatDOWN", "floatturnR", "floatturnL" ,
                                                        "float2fly", "flyforward" , "flyR", "flyL", "flyUP", "flyDOWN", "fly2float"};

            if (set == 5 && !phoneON) { addphone(); phoneON = true; }
            if (set != 5 && phoneON) { deletephone(); phoneON = false; }
            if (set == 3 || set == 7) rootONOFF(true);
        ...
        void changecharacter()
        {
            ...
            RandomMale.transform.position += transform.right * (0.8f + peopleofset) + Vector3.up * flyofset;
            RandomFemale.transform.position += transform.right * peopleofset + Vector3.up * flyofset;
            RandomElder.transform.position += transform.right * elderOfset + Vector3.up * flyofset;
            ...
            RandomMale.GetComponent<Animator>().Play("TPM_" + animations[animN]);
            RandomFemale.GetComponent<Animator>().Play("TPF_" + animations[animN]);
            if (elderON) RandomElder.GetComponent<Animator>().Play("TPE_" + animations[animN]);
            if (phoneON) addphone();
            if (elderN == 0) elderN++;
            else elderN = 0;
            if (set == 3) addchair();
        ...
        void addtallchair()
        {
            newtallchair1 = Instantiate(tallchair);
            newtallchair1.transform.position = transform.position + transform.right * 0.45f;

            newtallchair2 = Instantiate(tallchair);
            newtallchair2.transform.position = transform.position + transform.right * -0.35f;
        }
        void deletetallchair()
        {
            Destroy(newtallchair1);
            Destroy(newtallchair2);
        }
        void activeroot()
        {
            rootON = !rootON;
